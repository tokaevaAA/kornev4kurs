...
double func(double x){
    ...
    else if (strcmp(type_of_func,"myfunc2")==0){return sin(M_PI*5*x);}
...
double scal_proizv_fphi(double* mas_f, int n, int a, int N,  double h){
    double otv=0;
    for (int k=0; k<=N; k=k+1) otv=otv+mas_f[k]*sin(M_PI*n*(a+k*h));
    otv=otv/h;
    return otv;
...
double scal_proizv_phi_phi(int n, int a, int N,  double h){
    double otv=0;
    for (int k=0; k<=N; k=k+1) otv=otv+sin(M_PI*n*(a+k*h))*sin(M_PI*n*(a+k*h));
    otv=otv/h;
    return otv;
...
double trig_mnog(int N, double uzel, double* mas_c){
    double otv=0;
    for (int n=0; n<=N; n=n+1) otv=otv + mas_c[n]*sin(M_PI*n*uzel);
    return otv;
}
