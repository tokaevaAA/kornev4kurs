...
int generate_input(double a, double b, int n, char* tip_uzlov, const char* filename){
    ...
    if (strcmp(tip_uzlov,"ravnom")==0) {printf("ravnom uzl\n");
					...
					for (int i=0; i<=n-1; i=i+1) printf("%f ", a + i*h);
					printf("\n");
					for (int i=0; i<=n-1; i=i+1) printf("%f ", func(a + i*h));
					printf("\n");
					}
...
double P_n_1(int n, double x, double* B){
    double t=1.0;
    double otv=0;
    for (int i=0; i<=n-2; i=i+1){otv=otv+B[i]*t; t=t*x;} //last coefficient is h, we don't need it
    return otv;
...
int main(int argc, char** argv){
    ...
    if (argc != 6) {printf("Usage: ./a.out a b N=n+1 ravnom/chebushov myfunc/runge/modul \n"); return -1;}
    ...
    if (strcmp(argv[4],"ravnom")==0) {FILE*fout;
				      ...
				      for (int i=0; i<=m-1; i=i+1) {double uzel = a +i*h;
				 				     fprintf(fout,"%f %f %f %le \n", uzel, func(uzel), P_n_1(n,uzel,B),func(uzel)- P_n_1(n,uzel,B));
                                 				     printf("%f %f %f %le  \n", uzel, func(uzel), P_n_1(n,uzel,B), func(uzel)- P_n_1(n,uzel,B));
			      					     }
				      fclose(fout);
				
				      FILE*fout_gnuplot;
    				      fout_gnuplot=fopen("comands_for_gnuplot.txt","a");
				      if (fout_gnuplot==0) {printf("Cannot open commands_for_gnuplot.txt\n"); }
					
	
				      fprintf(fout_gnuplot, "set terminal png  size 640,480 \n");
				      fprintf(fout_gnuplot, "set output '%s.png' \n", output_name);
				      //fprintf(fout_gnuplot,"plot '%s' \n",output_name);
				      fprintf(fout_gnuplot,"plot '%s' using 1:2 with lines, '%s' using 1:3 with lines,  '%s' using 1:2 with points \n\n", 																		output_name,output_name,output_name);
				      ...
				      FILE* fout_h;
    				      fout_h=fopen("file_of_h.txt","a");
				      if (fout_h==0) {printf("Cannot open file_of_h.txt\n"); }
				      fprintf(fout_h,"%d %le\n",n, B(n));	
				      fclose(fout_h);
    ...
    if (strcmp(argv[4],"chebushov")==0){FILE*fout;
				        ...
				        for (int i=n; i>=1; i=i-1) {double uzel = mas1(i);
								     ...
								     if (i >=2){double h = (mas1(i)-mas1(i-1))/3;
										...
										fprintf(fout,"%f  %f %f %le\n", 
											dopuzel1, func(dopuzel1), P_n_1(n,dopuzel1,B),func(dopuzel1)- P_n_1(n,dopuzel1,B));
                                 				                printf("%f  %f %f %le\n", 
											dopuzel1, func(dopuzel1), P_n_1(n,dopuzel1,B),func(dopuzel1)- P_n_1(n,dopuzel1,B));
										fprintf(fout,"%f  %f %f %le \n", 
											dopuzel2, func(dopuzel2), P_n_1(n,dopuzel2,B),func(dopuzel2)- P_n_1(n,dopuzel2,B));
                                 				     		printf("%f  %f %f %le\n", 
											dopuzel2, func(dopuzel2), P_n_1(n,dopuzel2,B),func(dopuzel2)- P_n_1(n,dopuzel2,B));
										}
			      					    }
    ...
    //plot 'output_ravnom_uzel_func_P_n_1_L_n.txt' using 1:2 with lines, 'output_ravnom_uzel_func_P_n_1_L_n.txt' using 1:3 with lines, 
    //'output_ravnom_uzel_func_P_n_1_L_n.txt' using 1:4 with lines,  'output_ravnom_uzel_func_P_n_1_L_n.txt' using 1:2 with points 
    //plot 'output_chebushov_uzel_func_P_n_1_L_n.txt' using 1:2 with lines, 'output_chebushov_uzel_func_P_n_1_L_n.txt' using 1:3 with lines,    
     //'output_chebushov_uzel_func_P_n_1_L_n.txt' using 1:4 with lines
