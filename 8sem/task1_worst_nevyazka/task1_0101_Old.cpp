...
class SobstvFunc{
  ...
  public:
	SobstvFunc(int a_n, int a_N, double a_a, double  a_b){
			    ...
			    m_a=a_a;
			    m_b=a_b;
			    m_mas=static_cast<double*>(malloc((m_N+1)*sizeof(double)));
	...
	~SobstvFunc(){/*printf("Start deleting%p for n=%d\n",m_mas,m_n);*/free(m_mas); /*printf("End deleting\n");*/}

	//SobstvFunc(const SobstvFunc& sf)=delete;
	//SobstvFunc& operator=(const SobstvFunc& sf)=delete;
...
int main(int argc, char** argv){
    ...
    double* dop_mas=static_cast<double*>(malloc((N+1)*sizeof(double)));
